...
function TouchLayer:ctor()  
    ...
    listenner:registerScriptHandler(function(touch, event)  
        local locationInNodeX = self:convertToNodeSpace(touch:getLocation()).x  
        local locationInNodeY = self:convertToNodeSpace(touch:getLocation()).y  
        --点击屏蔽层，返回，相当于返回按钮
            self.dianji:getAnimation():playWithIndex(0,-1,-1)
            self.dianji:setPosition(cc.p(locationInNodeX,locationInNodeY))
    end, cc.Handler.EVENT_TOUCH_ENDED )  
...
Shieldingscreenmessage3 = class("Shieldingscreenmessage3", function ()  
    return cc.LayerColor:create(cc.c4b(0,0,0,0))  
end)  
