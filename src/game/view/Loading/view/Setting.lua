...
function Setting:Settingbtn( )
    ...
    local musiclabel = bg:getChildByName("music_label")
    local effectlabel = bg:getChildByName("effect_label")
    ...
    "duihao1.png","duihao1.png")         --创建菜单开的图片菜单项
     local soundOffItem = cc.MenuItemImage:create(
     "duihao2.png","duihao2.png")
     local soundItem = cc.MenuItemToggle:create(soundOnItem,soundOffItem)  --创建开关菜单项MenuIemToggle
     soundItem:setPosition(cc.p(effectlabel:getPositionX()-500,effectlabel:getPositionY()))
     ...
     local musicOnItem = cc.MenuItemImage:create("duihao1.png","duihao1.png")
     local musicOffItem = cc.MenuItemImage:create("duihao2.png","duihao2.png")
     local musicItem = cc.MenuItemToggle:create(musicOnItem,musicOffItem)
     musicItem:setPosition(cc.p(musiclabel:getPositionX()-500,musiclabel:getPositionY()))
    ...
    if UItool:getBool("effect",true) then
     ...
     local function percentChangedEvent(sender,eventType)
        if eventType == 0 then
            UItool:setInteger( "volumepercent" , sender:getPercent() )
            local volumepercent = sender:getPercent()/100
            AudioEngine.setMusicVolume(volumepercent) 
            AudioEngine.setEffectsVolume(volumepercent) 

        end
    end  
        
    local slider = bg:getChildByName("Slider")
    slider:setPercent(UItool:getInteger( "volumepercent",100))
